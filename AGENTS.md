...
- If the framework source/package cannot be accessed from a consuming repo, stop and report the blocker. Do not create local runtime shims or copied macros.
...
- Use documented `data-jbs-*` actions and component roots for interactivity. Do not invent alternate attributes such as `data-jbs-src` or `data-jbs-target-component`.
- Do not add framework-specific client state unless the repo explicitly opts into it.
...
- Cite the framework docs/source before claiming a primitive is missing.
